...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc != 5)
    ...
    m5_add_approx(  (uint32_t)&data, (uint32_t)(&data + (numOptions*sizeof(OptionData)) - 1), reliability_level);
    m5_add_approx(  (uint32_t)&prices, (uint32_t)(&prices + (numOptions*sizeof(fptype)) - 1), reliability_level);
...
#define LINESIZE 64
    ...
    m5_add_approx(  (uint32_t)&buffer, (uint32_t)(&buffer + (5 * numOptions * sizeof(fptype)) - 1), reliability_level);
