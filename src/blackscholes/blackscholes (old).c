...
#ifdef AMHM_APPROXIMATION
    m5_add_approx(  (uint32_t)&data[0], (uint32_t)(&data[numOptions] + sizeof(OptionData) - 1), reliability_level);
...
#ifdef AMHM_APPROXIMATION
    m5_add_approx(  (uint32_t)&buffer[0], (uint32_t)(&buffer[5 * numOptions] + sizeof(fptype) - 1), reliability_level);
#endif
