...
#ifdef AMHM_APPROXIMATION
    m5_add_approx(  (uint32_t)&buffer[0], (uint32_t)(&buffer[5 * numOptions] + PAD + sizeof(fptype) - 1), reliability_level);
    m5_add_approx(  (uint32_t)&sptprice[0], (uint32_t)(&sptprice[numOptions] - 1), reliability_level);
    m5_add_approx(  (uint32_t)&strike[0], (uint32_t)(&strike[numOptions] - 1), reliability_level);
    m5_add_approx(  (uint32_t)&rate[0], (uint32_t)(&rate[numOptions] - 1), reliability_level);
    m5_add_approx(  (uint32_t)&volatility[0], (uint32_t)(&volatility[numOptions] - 1), reliability_level);
    m5_add_approx(  (uint32_t)&otime[0], (uint32_t)(&otime[numOptions] - 1), reliability_level);
#endif
