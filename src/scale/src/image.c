...
int* read_image(char * filename, size_t* w, size_t* h, int *num_elems){
	FILE *fp;
	char header[16];
...
#ifdef SRC
    //m5_declare_mem_region(  (uint64_t)&data[0],       (uint64_t)(&data[*num_elems]+sizeof(int) - 1)      ); // MJ
	m5_add_approx((uint32_t)&data[0], (uint32_t)(&data[*num_elems]+sizeof(int) - 1), reliability_level);
#endif 
