...
typedef  struct LIST_OF_CORNERS {int x,y,info, dx, dy, I;} CORNER_LIST[MAX_CORNERS];
...
{
  //AMHM Start
  printf("Usage: susan <in.pgm> <out.pgm> [options] Reliability_Level\n\n");
  //AMHM End
...
/* }}} */
  ...
  tmp_in = (uchar *) malloc(*x_size * *y_size);
  
#ifdef get_image_IN 
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&tmp_in[0],       (uint64_t)(&tmp_in[*x_size * *y_size]+sizeof(uchar) - 1)      ); // MJ
  m5_add_approx((uint32_t)&tmp_in[0], (uint32_t)(&tmp_in[*x_size * *y_size]+sizeof(uchar) - 1), reliability_level);
  //AMHM End
#endif 
...
uchar *tmp_bp; // MJ
  ...
  tmp_bp=(unsigned char *)malloc(516);
  
//AMHM Start
printf("Man Injam 3.\n");
//AMHM End

#ifdef setup_brightness_lut_BP
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&tmp_bp[0],       (uint64_t)(&tmp_bp[516]+sizeof(unsigned char) - 1)      ); // MJ
  m5_add_approx((uint32_t)&tmp_bp[0], (uint32_t)(&tmp_bp[516]+sizeof(unsigned char) - 1), reliability_level);
  //AMHM End
#endif 
...
#ifdef susan_smoothing_TMP_IMAGE
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(tmp_image[0]),       (uint64_t)(&(tmp_image[(x_size+mask_size*2) * (y_size+mask_size*2)])+sizeof(uchar) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(tmp_image[0]), (uint32_t)(&(tmp_image[(x_size+mask_size*2) * (y_size+mask_size*2)])+sizeof(uchar) - 1), reliability_level);
  //AMHM End
#endif 
...
#ifdef susan_smoothing_DP
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(dp[0]),       (uint64_t)(&(dp[n_max*n_max])+sizeof(unsigned char) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(dp[0]), (uint32_t)(&(dp[n_max*n_max])+sizeof(unsigned char) - 1), reliability_level);
  //AMHM End
#endif 
...
#ifdef susan_corners_CGX
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(cgx[0]),       (uint64_t)(&(cgx[x_size*y_size])+sizeof(int) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(cgx[0]), (uint32_t)(&(cgx[x_size*y_size])+sizeof(int) - 1), reliability_level);
  //AMHM End
#endif 
  
  cgy=(int *)malloc(x_size*y_size*sizeof(int));
  
#ifdef susan_corners_CGY
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(cgy[0]),       (uint64_t)(&(cgy[x_size*y_size])+sizeof(int) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(cgy[0]), (uint32_t)(&(cgy[x_size*y_size])+sizeof(int) - 1), reliability_level);
  //AMHM End
#endif 
...
#ifdef susan_corners_CGX
//AMHM Start
  //m5_undeclare_mem_region(  (uint64_t)&(cgx[0]),       (uint64_t)(&(cgx[x_size*y_size])+sizeof(int) - 1)  ); // MJ
  m5_remove_approx((uint32_t)&(cgx[0]), (uint32_t)(&(cgx[x_size*y_size])+sizeof(int) - 1), 0);
//AMHM End
#endif 
  
#ifdef susan_corners_CGY
//AMHM Start
  //m5_undeclare_mem_region(  (uint64_t)&(cgy[0]),       (uint64_t)(&(cgy[x_size*y_size])+sizeof(int) - 1)  ); // MJ
  m5_remove_approx((uint32_t)&(cgy[0]), (uint32_t)(&(cgy[x_size*y_size])+sizeof(int) - 1), 0);
//AMHM End
#endif 
...
/* }}} */
  ...
  printf("Selected Reliability Level is: %d\n",reliability_level);
  //AMHM End
...
#ifdef main_EDGES_R
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(r[0]),       (uint64_t)(&(r[x_size * y_size])+sizeof(int) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(r[0]), (uint32_t)(&(r[x_size * y_size])+sizeof(int) - 1), reliability_level);
  //AMHM End
#endif 
...
#ifdef main_MID
  //AMHM Start
  //m5_declare_mem_region(  (uint64_t)&(mid[0]),       (uint64_t)(&(mid[x_size * y_size])+sizeof(uchar) - 1)      ); // MJ
  m5_add_approx((uint32_t)&(mid[0]), (uint32_t)(&(mid[x_size * y_size])+sizeof(uchar) - 1), reliability_level);
  //AMHM End
#endif 
